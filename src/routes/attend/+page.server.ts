...
export const load = (async ({ locals }) => {
	...
	// Load events from database; fall back to hardcoded list if none exist
	const dbEvents = await locals.db.query.events.findMany({
	...
	const eventList =
		dbEvents.length > 0
			? dbEvents.map((e) => ({
					name: e.name,
					dateStr: e.dateStr,
					smugmugAlbumUrl: e.smugmugAlbumUrl
				}))
			: HARDCODED_EVENTS;
